	...
	case CL_PLATFORM_VERSION:
		{
			...
			strcpy_s((char*)param_value, 1024, spDriverVersion);
		...
		case CL_PLATFORM_VENDOR	:
			...
			strcpy_s((char*)param_value, 1024, spPlatformID);
			if(param_value_size_ret != NULL)
			{
	...
	size_t *                param_value_size_ret) CL_API_SUFFIX__VERSION_1_0
{
	...
	case CL_DEVICE_NAME:
		{
			...
			if (nameLen < param_value_size)
			{
				...
				sprintf_s((char*)param_value,1024,"%s",cpuName);
			} else
			{
	...
	case CL_DEVICE_MAX_WORK_ITEM_SIZES:
		{
			size_t workitem_size[3]; workitem_size[0] = 0; // [dummy to remove warning]
               ...
               cl_int *         errcode_ret ) CL_API_SUFFIX__VERSION_1_0
{
	...
	strcpy_s(kernel->m_name, 1024, kernel_name);
	kernel->m_numArgs = 0;
	...
	case CL_CONTEXT_DEVICES:
		{
			...
			} else
			{
				...
				sprintf_s((char*)param_value,1024,"%s",testName);
			}
			break;
		};
	default:
